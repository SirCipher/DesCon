...
uint16_t STMPE811_ReadX(uint16_t x);
uint16_t STMPE811_ReadY(uint16_t y);
...
STMPE811_State_t STMPE811_Init(void) {
	...
	DELAY_Init();
	/* Initialize I2C */
	I2C_Init(STMPE811_I2C, STMPE811_I2C_PINSPACK, STMPE811_I2C_CLOCK);
	
	/* Reset */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL1, 0x02);
	Delayms(5);
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL1, 0x00);
	Delayms(2);
	
	/* Check for STMPE811 Connected */
	I2C_ReadMulti(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_CHIP_ID, bytes, 2);
	...
	mode &= ~(0x01); 
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL2, mode);
	mode = STMPE811_Read(STMPE811_SYS_CTRL2);
	mode &= ~(0x02); 
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL2, mode); 

	/* Select Sample Time, bit number and ADC Reference */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_ADC_CTRL1, 0x49);

	/* Wait for 2 ms */
	Delayms(2); 

	/* Select the ADC clock speed: 3.25 MHz */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_ADC_CTRL2, 0x01);

	/* Select TSC pins in non default mode */  
	mode = STMPE811_Read(STMPE811_GPIO_AF);
	mode |= 0x1E;
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_GPIO_AF, mode);
	...
	*/
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_CFG, 0x9A);

	/* Configure the Touch FIFO threshold: single point reading */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_TH, 0x01);

	/* Clear the FIFO memory content. */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_STA, 0x01);

	/* Put the FIFO back into operation mode  */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_STA, 0x00);

	/* Set the range and accuracy pf the pressure measurement (Z) : 
	- Fractional part :7 
	- Whole part      :1 
	*/
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_FRACTION_Z, 0x01);

	/* Set the driving capability (limit) of the device for TSC pins: 50mA */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_I_DRIVE, 0x01);

	/* Touch screen control configuration (enable TSC):
	- No window tracking index
	- XYZ acquisition mode
	*/
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_CTRL, 0x03);

	/* Clear all the status pending bits if any */
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_INT_STA, 0xFF);
	
	/* Enable global interrupts */
	/*
	mode = I2C_Read(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_INT_CTRL);
	mode |= 0x01;
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_INT_CTRL, mode);
	*/
	/* Enable touch interrupt */
	/*
	mode = I2C_Read(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_INT_EN);
	mode |= 0x01;
	I2C_Write(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_INT_EN, mode);
	*/
	/* Wait for 2 ms delay */
	Delayms(2); 
...
uint8_t STMPE811_Read(uint8_t reg) {
	return I2C_Read(STMPE811_I2C, STMPE811_ADDRESS, reg);
}

STMPE811_State_t STMPE811_ReadTouch(STMPE811_TouchData *structdata) {
	uint8_t val;
	...
	if (structdata->orientation == STMPE811_Orientation_Portrait_1) {
		structdata->x = 239 - STMPE811_ReadX(structdata->x);
		structdata->y = 319 - STMPE811_ReadY(structdata->y);
	} else if (structdata->orientation == STMPE811_Orientation_Portrait_2) {
		structdata->x = STMPE811_ReadX(structdata->x);
		structdata->y = STMPE811_ReadY(structdata->y);
	} else if (structdata->orientation == STMPE811_Orientation_Landscape_1) {
		structdata->y = STMPE811_ReadX(structdata->y);
		structdata->x = 319 - STMPE811_ReadY(structdata->x);
	} else if (structdata->orientation == STMPE811_Orientation_Landscape_2) {
		structdata->y = 239 - STMPE811_ReadX(structdata->x);
		structdata->x = STMPE811_ReadY(structdata->x);
	}
