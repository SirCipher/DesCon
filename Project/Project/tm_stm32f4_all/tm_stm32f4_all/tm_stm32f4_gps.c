...
static uint8_t GPS_CRC, GPS_CRC_Received;
static uint8_t GPS_Star;
static uint8_t GPS_Statement = GPS_ERR;
static uint32_t GPS_Flags = 0, GPS_Flags_OK;
static GPS_Data_t GPS_INT_Data;
static uint8_t GPS_FirstTime;
...
/* Private */
GPS_Result_t GPS_INT_Do(GPS_t* GPS_Data, char c);
void GPS_INT_CheckTerm(GPS_t* GPS_Data);
GPS_Result_t GPS_INT_Return(GPS_t* GPS_Data);
uint8_t GPS_INT_StringStartsWith(char* string, const char* str);
uint8_t GPS_INT_Atoi(char* str, uint32_t* val);
uint32_t GPS_INT_Pow(uint8_t x, uint8_t y);
uint8_t GPS_INT_Hex2Dec(char c);
uint8_t GPS_INT_FlagsOk(GPS_t* GPS_Data);
void GPS_INT_ClearFlags(GPS_t* GPS_Data);
void GPS_INT_CheckEmpty(GPS_t* GPS_Data);

#define GPS_INT_Add2CRC(c)                            (GPS_CRC ^= c)
#define GPS_INT_ReturnWithStatus(GPS_Data, status)    (GPS_Data)->Status = status; return status;
#define GPS_INT_SetFlag(flag)                         (GPS_Flags |= (flag))
...
GPS_Result_t GPS_Update(GPS_t* GPS_Data) {
	...
	if (!GPS_USART_BUFFER_EMPTY) {
		...
		while (!GPS_USART_BUFFER_EMPTY) {
			...
			GPS_INT_Do(GPS_Data, (char)GPS_USART_BUFFER_GET_CHAR);
			/* If new data available, return to user */
			if (GPS_Data->Status == GPS_Result_NewData) {
	...
	if (GPS_FirstTime) {
		...
		GPS_INT_ReturnWithStatus(GPS_Data, GPS_Result_FirstDataWaiting);
	}
	
	/* We have old data */
	GPS_INT_ReturnWithStatus(GPS_Data, GPS_Result_OldData);
}
...
GPS_Custom_t * GPS_AddCustom(GPS_t* GPS_Data, char* GPG_Statement, uint8_t TermNumber) {
	...
	/* Allocate memory */
	temp = (GPS_Custom_t *) malloc(sizeof(GPS_Custom_t));
...
void GPS_ConvertFloat(float num, GPS_Float_t* Float_Data, uint8_t decimals) {
	...
	/* Get decimal part */
	if (num < 0) {
		Float_Data->Decimal = (int32_t)((float)(Float_Data->Integer - num) * GPS_INT_Pow(10, decimals));
	} else {
		Float_Data->Decimal = (int32_t)((float)(num - Float_Data->Integer) * GPS_INT_Pow(10, decimals));
	}
}
...
GPS_Result_t GPS_INT_Do(GPS_t* GPS_Data, char c) {
	...
	if (c == '$') {
		...
		/* Check empty */
		GPS_INT_CheckEmpty(GPS_Data);
		
		/* Check term */
		GPS_INT_CheckTerm(GPS_Data);
		...
		if (GPS_Statement == GPS_GPGSV && GPGSV_StatementsCount == GPSGV_StatementNumber) {
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_SATSDESC);
		}
#endif
	} else if (c == '\r') {
		GPS_Term[GPS_Term_Pos++] = 0;
		...
		GPS_CRC_Received = GPS_INT_Hex2Dec(GPS_Term[0]) * 16 + GPS_INT_Hex2Dec(GPS_Term[1]);
		
		if (GPS_CRC_Received != GPS_CRC) {
	...
	/* Return */
	return GPS_INT_Return(GPS_Data);
}
...
void GPS_INT_CheckTerm(GPS_t* GPS_Data) {
	...
	if (GPS_Term_Number == 0) {
		/* Statement indicator */
		if (GPS_INT_StringStartsWith(GPS_Term, "$GPGGA")) {
			GPS_Statement = GPS_GPGGA;
		} else if (GPS_INT_StringStartsWith(GPS_Term, "$GPRMC")) {
			GPS_Statement = GPS_GPRMC;
		} else if (GPS_INT_StringStartsWith(GPS_Term, "$GPGSA")) {
			GPS_Statement = GPS_GPGSA;
		} else if (GPS_INT_StringStartsWith(GPS_Term, "$GPGSV")) {
			GPS_Statement = GPS_GPGSV;
		} else {
			GPS_Statement = GPS_ERR;
		}
	...
	for (i = 0; i < GPS_Data->CustomStatementsCount; i++) {
		...
		if (GPS_INT_StringStartsWith(GPS_Statement_Name, GPS_Data->CustomStatements[i]->Statement)) {
	...
	switch (GPS_CONCAT(GPS_Statement, GPS_Term_Number)) {
#ifndef GPS_DISABLE_GPGGA
		case GPS_POS_LATITUDE:	/* GPGGA */
			/* Convert latitude */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Latitude = temp / 100;
			GPS_INT_Data.Latitude += (float)(temp % 100) / (float)60;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.Latitude += temp / (GPS_INT_Pow(10, count) * 60.0);
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_LATITUDE);
			break;
		case GPS_POS_NS: /* GPGGA */
			if (GPS_Term[0] == 'S') {
				/* South has negative coordinate */
				GPS_INT_Data.Latitude = -GPS_INT_Data.Latitude;
			}
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_NS);
			break;
		case GPS_POS_LONGITUDE: /* GPGGA */
			/* Convert longitude */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Longitude = temp / 100; /* Degrees */
			GPS_INT_Data.Longitude += (float)(temp % 100) / (float)60;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.Longitude += temp / (GPS_INT_Pow(10, count) * 60.0);
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_LONGITUDE);
			break;
		case GPS_POS_EW: /* GPGGA */
			if (GPS_Term[0] == 'W') {
				/* West has negative coordinate */
				GPS_INT_Data.Longitude = -GPS_INT_Data.Longitude;
			}
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_EW);
			break;
		case GPS_POS_SATS: /* GPGGA */
			/* Satellites in use */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Satellites = temp;
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_SATS);
			break;
		case GPS_POS_FIX: /* GPGGA */
			/* GPS Fix */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Fix = temp;	
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_FIX);
			break;
		case GPS_POS_ALTITUDE: /* GPGGA */
			/* Convert altitude above sea */
			if (GPS_Term[0] == '-') {
				count = GPS_INT_Atoi(&GPS_Term[1], &temp);
				GPS_INT_Data.Altitude = temp;
				
				count++;
				count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
				GPS_INT_Data.Altitude += temp / (GPS_INT_Pow(10, count) * 1.0);
				
				GPS_INT_Data.Altitude = -GPS_INT_Data.Altitude;
			} else {
				count = GPS_INT_Atoi(GPS_Term, &temp);
				GPS_INT_Data.Altitude = temp;
			...
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_ALTITUDE);
			break;
		case GPS_POS_TIME: /* GPGGA */
			/* Set time */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Time.Seconds = temp % 100;
			GPS_INT_Data.Time.Minutes = (int)(temp * (float) 0.01) % 100;
			GPS_INT_Data.Time.Hours = (int)(temp * (float)0.0001) % 100;
			/* Hundredths */
			GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.Time.Hundredths = temp;
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_TIME);
			break;
#endif
#ifndef GPS_DISABLE_GPRMC
		case GPS_POS_SPEED:	/* GPRMC */	
			/* Convert speed */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Speed = (float)temp;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.Speed += (float)((float)temp / (GPS_INT_Pow(10, count) * 1.0));
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_SPEED);
			break;
		case GPS_POS_DATE: /* GPRMC */	
			/* Set date */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Date.Year = temp % 100;
			GPS_INT_Data.Date.Month = (int)(temp * (float)0.01) % 100;
			GPS_INT_Data.Date.Date = (int)(temp * (float) 0.0001) % 100;
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_DATE);
			break;
		case GPS_POS_VALIDITY: /* GPRMC */	
			/* GPS valid status */
			GPS_INT_Data.Validity = GPS_Term[0] == 'A';
			
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_VALIDITY);
			break;
		case GPS_POS_DIRECTION: /* GPRMC */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.Direction = (float)temp;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.Direction += (float)((float)temp / (GPS_INT_Pow(10, count) * 1.0));
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_DIRECTION);
			break;
#endif
#ifndef GPS_DISABLE_GPGSA
		case GPS_POS_HDOP: /* GPGSA */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.HDOP = (float)temp;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.HDOP += (float)((float)temp / (GPS_INT_Pow(10, count) * 1.0));
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_HDOP);
			break;
		case GPS_POS_PDOP: /* GPGSA */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.PDOP = (float)temp;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.PDOP += (float)((float)temp / (GPS_INT_Pow(10, count) * 1.0));
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_PDOP);
			break;
		case GPS_POS_VDOP: /* GPGSA */
			count = GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.VDOP = (float)temp;
		
			count = GPS_INT_Atoi(&GPS_Term[++count], &temp);
			GPS_INT_Data.VDOP += (float)((float)temp / (GPS_INT_Pow(10, count) * 1.0));
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_VDOP);
			break;
		case GPS_POS_FIXMODE: /* GPGSA */
			/* Satellites in view */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.FixMode = temp;
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_FIXMODE);
			break;
		case GPS_POS_SAT1:
		case GPS_POS_SAT2:
		case GPS_POS_SAT3:
		...
		case GPS_POS_SAT12:
			/* Satellite numbers */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.SatelliteIDs[GPS_Term_Number - 3] = temp;
		
			/* Increase number of satellites found */
			ids_count++;
		
			if (ids_count == GPS_INT_Data.Satellites) {
				ids_count = 0;
				
				/* Set flag */
				GPS_INT_SetFlag(GPS_FLAG_SATS1_12);
			}
			break;
#endif
#ifndef GPS_DISABLE_GPGSV
		case GPS_POS_SATSINVIEW: /* GPGSV */
			/* Satellites in view */
			GPS_INT_Atoi(GPS_Term, &temp);
			GPS_INT_Data.SatellitesInView = temp;
		
			/* Set flag */
			GPS_INT_SetFlag(GPS_FLAG_SATSINVIEW);
			break;
#endif
		default: 
			break;
	...
	if (GPS_Statement == GPS_GPGSV) {
		...
		if (GPS_Term_Number >= 4) {
			...
			if (GPGSV_Term_Number < GPS_MAX_SATS_IN_VIEW) {
				/* Check offset from 4 */
				if (GPGSV_Term_Mod == 0) {
					GPS_INT_Data.SatDesc[GPGSV_Term_Number].ID = temp;
				} else if (GPGSV_Term_Mod == 1) {
					GPS_INT_Data.SatDesc[GPGSV_Term_Number].Elevation = temp;
				} else if (GPGSV_Term_Mod == 2) {
					GPS_INT_Data.SatDesc[GPGSV_Term_Number].Azimuth = temp;
				} else if (GPGSV_Term_Mod == 3) {
					GPS_INT_Data.SatDesc[GPGSV_Term_Number].SNR = temp;
				}
			}
		}
	}
...
GPS_Result_t GPS_INT_Return(GPS_t* GPS_Data) {
	...
	if (GPS_INT_FlagsOk(GPS_Data)) {
		...
		/* Set data */
#ifndef GPS_DISABLE_GPGGA
		GPS_Data->Latitude = GPS_INT_Data.Latitude;
		GPS_Data->Longitude = GPS_INT_Data.Longitude;
		GPS_Data->Satellites = GPS_INT_Data.Satellites;
		GPS_Data->Fix = GPS_INT_Data.Fix;
		GPS_Data->Altitude = GPS_INT_Data.Altitude;
		GPS_Data->Time = GPS_INT_Data.Time;
#endif
#ifndef GPS_DISABLE_GPRMC
		GPS_Data->Speed = GPS_INT_Data.Speed;
		GPS_Data->Date = GPS_INT_Data.Date;
		GPS_Data->Validity = GPS_INT_Data.Validity;
		GPS_Data->Direction = GPS_INT_Data.Direction;
#endif
#ifndef GPS_DISABLE_GPGSA
		GPS_Data->HDOP = GPS_INT_Data.HDOP;
		GPS_Data->VDOP = GPS_INT_Data.VDOP;
		GPS_Data->PDOP = GPS_INT_Data.PDOP;
		GPS_Data->FixMode = GPS_INT_Data.FixMode;
		for (i = 0; i < 12; i++) {
			GPS_Data->SatelliteIDs[i] = GPS_INT_Data.SatelliteIDs[i];
		}
#endif
#ifndef GPS_DISABLE_GPGSV
		GPS_Data->SatellitesInView = GPS_INT_Data.SatellitesInView;
		for (i = 0; i < GPS_MAX_SATS_IN_VIEW; i++) {
			GPS_Data->SatDesc[i] = GPS_INT_Data.SatDesc[i];
		}
#endif
		
		/* Return new data */
		GPS_INT_ReturnWithStatus(GPS_Data, GPS_Result_NewData);
	}
